  ...
  namespace arch {
    namespace x86 {
      ...
      triton::uint64 x86Semantics::alignAddStack_s(triton::arch::Instruction& inst, triton::uint32 delta) {
        auto dst = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_SP));
      ...
      void x86Semantics::controlFlow_s(triton::arch::Instruction& inst) {
        auto pc      = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_IP));
        auto counter = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_CX));
        auto zf      = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ZF));
        ...
        switch (inst.getPrefix()) {
          ...
          default: {
            ...
            /* Create symbolic expression */
            auto expr = this->symbolicEngine->createSymbolicRegisterExpression(inst, node, architecture->getParentRegister(ID_REG_X86_IP), "Program Counter");

            /* Spread taint */
            expr->isTainted = this->taintEngine->setTaintRegister(architecture->getParentRegister(ID_REG_X86_IP), triton::engines::taint::UNTAINTED);
            break;
          }
        }
      }
      ...
      void x86Semantics::af_s(triton::arch::Instruction& inst,
        ...
        auto expr = this->symbolicEngine->createSymbolicFlagExpression(inst, node, architecture->getRegister(ID_REG_X86_AF), "Adjust flag");

        /* Spread the taint from the parent to the child */
        expr->isTainted = this->taintEngine->setTaintRegister(architecture->getRegister(ID_REG_X86_AF), parent->isTainted);
      }
      ...
      void x86Semantics::cfAdd_s(triton::arch::Instruction& inst,
        ...
        /* Create the symbolic expression */
        auto expr = this->symbolicEngine->createSymbolicFlagExpression(inst, node, architecture->getRegister(ID_REG_X86_CF), "Carry flag");

        /* Spread the taint from the parent to the child */
        expr->isTainted = this->taintEngine->setTaintRegister(architecture->getRegister(ID_REG_X86_CF), parent->isTainted);
      }
      ...
      void x86Semantics::cfRcl_s(triton::arch::Instruction& inst,
        ...
        auto cf     = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_CF));
      ...
      void x86Semantics::ofAdd_s(triton::arch::Instruction& inst,
        ...
        /* Create the symbolic expression */
        auto expr = this->symbolicEngine->createSymbolicFlagExpression(inst, node, architecture->getRegister(ID_REG_X86_OF), "Overflow flag");

        /* Spread the taint from the parent to the child */
        expr->isTainted = this->taintEngine->setTaintRegister(architecture->getRegister(ID_REG_X86_OF), parent->isTainted);
      }
      ...
      void x86Semantics::ofRol_s(triton::arch::Instruction& inst,
        ...
        auto high   = vol ? bvSize-1 : dst.getAbstractHigh();
        auto cf     = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_CF));
        auto of     = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_OF));
      ...
      void x86Semantics::pf_s(triton::arch::Instruction& inst,
        ...
        /* Create the symbolic expression */
        auto expr = this->symbolicEngine->createSymbolicFlagExpression(inst, node, architecture->getRegister(ID_REG_X86_PF), "Parity flag");

        /* Spread the taint from the parent to the child */
        expr->isTainted = this->taintEngine->setTaintRegister(architecture->getRegister(ID_REG_X86_PF), parent->isTainted);
      }
      ...
      void x86Semantics::pfShl_s(triton::arch::Instruction& inst,
        ...
        auto low    = vol ? 0 : dst.getAbstractLow();
        auto high   = vol ? BYTE_SIZE_BIT-1 : !low ? BYTE_SIZE_BIT-1 : WORD_SIZE_BIT-1;
        auto pf     = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_PF));
        ...
        /* Create the symbolic expression */
        auto expr = this->symbolicEngine->createSymbolicFlagExpression(inst, node2, architecture->getRegister(ID_REG_X86_PF), "Parity flag");
      ...
      void x86Semantics::sf_s(triton::arch::Instruction& inst,
        ...
        /* Create the symbolic expression */
        auto expr = this->symbolicEngine->createSymbolicFlagExpression(inst, node, architecture->getRegister(ID_REG_X86_SF), "Sign flag");

        /* Spread the taint from the parent to the child */
        expr->isTainted = this->taintEngine->setTaintRegister(architecture->getRegister(ID_REG_X86_SF), parent->isTainted);
      }
      ...
      void x86Semantics::sfShl_s(triton::arch::Instruction& inst,
        ...
        auto bvSize = dst.getBitSize();
        auto high   = vol ? bvSize-1 : dst.getAbstractHigh();
        auto sf     = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_SF));
      ...
      void x86Semantics::zf_s(triton::arch::Instruction& inst,
        ...
        /* Create the symbolic expression */
        auto expr = this->symbolicEngine->createSymbolicFlagExpression(inst, node, architecture->getRegister(ID_REG_X86_ZF), "Zero flag");

        /* Spread the taint from the parent to the child */
        expr->isTainted = this->taintEngine->setTaintRegister(architecture->getRegister(ID_REG_X86_ZF), parent->isTainted);
      }
      ...
      void x86Semantics::aad_s(triton::arch::Instruction& inst) {
        ...
        auto  src2   = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AL));
        auto  src3   = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AH));
        auto  dst    = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AX));
        auto  dsttmp = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AL));
      ...
      void x86Semantics::and_s(triton::arch::Instruction& inst) {
        ...
        /* Upate symbolic flags */
        this->clearFlag_s(inst, architecture->getRegister(ID_REG_X86_CF), "Clears carry flag");
        this->clearFlag_s(inst, architecture->getRegister(ID_REG_X86_OF), "Clears overflow flag");
        this->pf_s(inst, expr, dst);
        this->sf_s(inst, expr, dst);
        this->zf_s(inst, expr, dst);
      ...
      void x86Semantics::blsmsk_s(triton::arch::Instruction& inst) {
        ...
        this->clearFlag_s(inst, architecture->getRegister(ID_REG_X86_OF), "Clears overflow flag");
        this->sf_s(inst, expr, dst);
        this->clearFlag_s(inst, architecture->getRegister(ID_REG_X86_ZF), "Clears zero flag");
      ...
      void x86Semantics::bt_s(triton::arch::Instruction& inst) {
        auto  dst  = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_CF));
        ...
        /* Create symbolic expression */
        auto expr = this->symbolicEngine->createSymbolicFlagExpression(inst, node, architecture->getRegister(ID_REG_X86_CF), "BT operation");
      ...
      void x86Semantics::btc_s(triton::arch::Instruction& inst) {
        auto  dst1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_CF));
        ...
        /* Create symbolic expression */
        auto expr1 = this->symbolicEngine->createSymbolicFlagExpression(inst, node1, architecture->getRegister(ID_REG_X86_CF), "BTC carry operation");
      ...
      void x86Semantics::btr_s(triton::arch::Instruction& inst) {
        ...
        /* Create symbolic expression */
        auto expr1 = this->symbolicEngine->createSymbolicFlagExpression(inst, node1, architecture->getRegister(ID_REG_X86_CF), "BTR carry operation");
      ...
      void x86Semantics::bts_s(triton::arch::Instruction& inst) {
        ...
        /* Create symbolic expression */
        auto expr1 = this->symbolicEngine->createSymbolicFlagExpression(inst, node1, architecture->getRegister(ID_REG_X86_CF), "BTS carry operation");
      ...
      void x86Semantics::call_s(triton::arch::Instruction& inst) {
        auto stack = architecture->getParentRegister(ID_REG_X86_SP);
      ...
      void x86Semantics::cdq_s(triton::arch::Instruction& inst) {
        auto dst = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EDX));
        auto src = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EAX));
        ...
        /* Spread taint */
        expr1->isTainted = this->taintEngine->isRegisterTainted(architecture->getRegister(ID_REG_X86_EAX));
      ...
      void x86Semantics::cdqe_s(triton::arch::Instruction& inst) {
        auto dst = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RAX));
      ...
      void x86Semantics::cld_s(triton::arch::Instruction& inst) {
        this->clearFlag_s(inst, architecture->getRegister(ID_REG_X86_DF), "Clears direction flag");
        /* Upate the symbolic control flow */
        this->controlFlow_s(inst);
      }
      ...
      void x86Semantics::clts_s(triton::arch::Instruction& inst) {
        auto dst = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_CR0));
      ...
      void x86Semantics::cli_s(triton::arch::Instruction& inst) {
        this->clearFlag_s(inst, architecture->getRegister(ID_REG_X86_IF), "Clears interrupt flag");
        /* Upate the symbolic control flow */
        this->controlFlow_s(inst);
      }
      ...
      void x86Semantics::cmpsb_s(triton::arch::Instruction& inst) {
        auto& dst    = inst.operands[0];
        auto& src    = inst.operands[1];
        auto  index1 = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_SI));
        auto  index2 = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_DI));
        auto  cx     = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_CX));
        auto  df     = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_DF));
      ...
      void x86Semantics::cmpxchg_s(triton::arch::Instruction& inst) {
        ...
        triton::arch::OperandWrapper accumulator(architecture->getRegister(ID_REG_X86_AL));
        triton::arch::OperandWrapper accumulatorp(architecture->getParentRegister(ID_REG_X86_AL));

        switch (src1.getSize()) {
          case WORD_SIZE:
            accumulator.setRegister(arch::Register(architecture->getRegister(ID_REG_X86_AX)));
            break;
          case DWORD_SIZE:
            accumulator.setRegister(arch::Register(architecture->getRegister(ID_REG_X86_EAX)));
            break;
          case QWORD_SIZE:
            accumulator.setRegister(arch::Register(architecture->getRegister(ID_REG_X86_RAX)));
            break;
        }
      ...
      void x86Semantics::cmpxchg16b_s(triton::arch::Instruction& inst) {
        auto& src1 = inst.operands[0];
        auto  src2 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RDX));
        auto  src3 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RAX));
        auto  src4 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RCX));
        auto  src5 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RBX));
      ...
      void x86Semantics::cmpxchg8b_s(triton::arch::Instruction& inst) {
        auto& src1  = inst.operands[0];
        auto  src2  = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EDX));
        auto  src3  = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EAX));
        auto  src4  = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ECX));
        auto  src5  = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EBX));
        auto  src2p = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_EDX));
        auto  src3p = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_EAX));
      ...
      void x86Semantics::cpuid_s(triton::arch::Instruction& inst) {
        auto src  = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_AX));
        auto dst1 = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_AX));
        auto dst2 = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_BX));
        auto dst3 = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_CX));
        auto dst4 = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_DX));
        ...
        /* Spread taint */
        expr1->isTainted = this->taintEngine->setTaintRegister(architecture->getParentRegister(ID_REG_X86_AX), false);
        expr2->isTainted = this->taintEngine->setTaintRegister(architecture->getParentRegister(ID_REG_X86_BX), false);
        expr3->isTainted = this->taintEngine->setTaintRegister(architecture->getParentRegister(ID_REG_X86_CX), false);
        expr4->isTainted = this->taintEngine->setTaintRegister(architecture->getParentRegister(ID_REG_X86_DX), false);
      ...
      void x86Semantics::cqo_s(triton::arch::Instruction& inst) {
        auto dst = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RDX));
        auto src = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RAX));
        ...
        /* Spread taint */
        expr1->isTainted = this->taintEngine->isRegisterTainted(architecture->getRegister(ID_REG_X86_RAX));
      ...
      void x86Semantics::cwd_s(triton::arch::Instruction& inst) {
        auto dst = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_DX));
        auto src = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AX));
        ...
        /* Spread taint */
        expr1->isTainted = this->taintEngine->isRegisterTainted(architecture->getRegister(ID_REG_X86_AX));
      ...
      void x86Semantics::cwde_s(triton::arch::Instruction& inst) {
        auto dst = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EAX));
      ...
      void x86Semantics::div_s(triton::arch::Instruction& inst) {
        ...
        switch (src.getSize()) {
          ...
          case BYTE_SIZE: {
            /* AX */
            auto ax = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AX));
          ...
          case WORD_SIZE: {
            /* DX:AX */
            auto dx = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_DX));
            auto ax = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AX));
          ...
          case DWORD_SIZE: {
            /* EDX:EAX */
            auto edx = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EDX));
            auto eax = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EAX));
          ...
          case QWORD_SIZE: {
            /* RDX:RAX */
            auto rdx = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RDX));
            auto rax = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RAX));
        ...
        switch (inst.operands.size()) {
          ...
          case 1: {
            ...
            switch (src.getSize()) {
              ...
              case BYTE_SIZE: {
                auto ax   = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AX));
                auto al   = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AL));
      ...
      void x86Semantics::lahf_s(triton::arch::Instruction& inst) {
        auto dst  = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AH));
        auto src1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_SF));
        auto src2 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ZF));
        auto src3 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AF));
        auto src4 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_PF));
        auto src5 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_CF));
      ...
      void x86Semantics::ldmxcsr_s(triton::arch::Instruction& inst) {
        auto  dst = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_MXCSR));
        auto& src = inst.operands[0];
      ...
      void x86Semantics::lea_s(triton::arch::Instruction& inst) {
        ...
        /* Base with PC */
        if (this->architecture->isRegisterValid(srcBase) && (architecture->getParentRegister(srcBase) == architecture->getParentRegister(ID_REG_X86_IP)))
      ...
      void x86Semantics::leave_s(triton::arch::Instruction& inst) {
        auto stack     = architecture->getParentRegister(ID_REG_X86_SP);
        auto base      = architecture->getParentRegister(ID_REG_X86_BP);
        ...
        auto bp2       = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_BP));
      ...
      void x86Semantics::lodsb_s(triton::arch::Instruction& inst) {
        auto& dst    = inst.operands[0];
        auto& src    = inst.operands[1];
        auto  index  = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_SI));
        auto  cx     = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_CX));
        auto  df     = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_DF));
      ...
      void x86Semantics::movsb_s(triton::arch::Instruction& inst) {
        auto& dst    = inst.operands[0];
        auto& src    = inst.operands[1];
        auto  index1 = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_DI));
        auto  index2 = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_SI));
        auto  cx     = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_CX));
        auto  df     = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_DF));
      ...
      void x86Semantics::mul_s(triton::arch::Instruction& inst) {
        ...
        switch (src2.getSize()) {
          ...
          case BYTE_SIZE: {
            auto dst  = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AX));
            auto src1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AL));
            /* Create symbolic operands */
            auto op1 = this->symbolicEngine->buildSymbolicOperand(inst, src1);
            auto op2 = this->symbolicEngine->buildSymbolicOperand(inst, src2);
            /* Create the semantics */
          ...
          /* DX:AX = AX * r/m16 */
          case WORD_SIZE: {
            auto dst1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AX));
            auto dst2 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_DX));
            auto src1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AX));
            /* Create symbolic operands */
            auto op1 = this->symbolicEngine->buildSymbolicOperand(inst, src1);
            auto op2 = this->symbolicEngine->buildSymbolicOperand(inst, src2);
            /* Create the semantics */
          ...
          /* EDX:EAX = EAX * r/m32 */
          case DWORD_SIZE: {
            auto dst1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EAX));
            auto dst2 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EDX));
            auto src1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EAX));
            /* Create symbolic operands */
            auto op1 = this->symbolicEngine->buildSymbolicOperand(inst, src1);
            auto op2 = this->symbolicEngine->buildSymbolicOperand(inst, src2);
            /* Create the semantics */
          ...
          /* RDX:RAX = RAX * r/m64 */
          case QWORD_SIZE: {
            auto dst1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RAX));
            auto dst2 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RDX));
            auto src1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RAX));
            /* Create symbolic operands */
            auto op1 = this->symbolicEngine->buildSymbolicOperand(inst, src1);
            auto op2 = this->symbolicEngine->buildSymbolicOperand(inst, src2);
            /* Create the semantics */
      ...
      void x86Semantics::popal_s(triton::arch::Instruction& inst) {
        auto stack      = architecture->getParentRegister(ID_REG_X86_SP);
        auto stackValue = this->architecture->getConcreteRegisterValue(stack).convert_to<triton::uint64>();
        auto dst1       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EDI));
        auto dst2       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ESI));
        auto dst3       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EBP));
        auto dst4       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ESP));
        auto dst5       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EBX));
        auto dst6       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EDX));
        auto dst7       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ECX));
        auto dst8       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EAX));
      ...
      void x86Semantics::popfd_s(triton::arch::Instruction& inst) {
        ...
        auto  stackValue = this->architecture->getConcreteRegisterValue(stack).convert_to<triton::uint64>();
        auto  dst1       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_CF));
        auto  dst2       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_PF));
        auto  dst3       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AF));
        auto  dst4       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ZF));
        auto  dst5       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_SF));
        auto  dst6       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_TF));
        auto  dst7       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_IF));
        auto  dst8       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_DF));
        auto  dst9       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_OF));
      ...
      void x86Semantics::ptest_s(triton::arch::Instruction& inst) {
        ...
        /* Upate symbolic flags */
        this->clearFlag_s(inst, architecture->getRegister(ID_REG_X86_AF), "Clears adjust flag");
        this->cfPtest_s(inst, expr2, src1, true);
        this->clearFlag_s(inst, architecture->getRegister(ID_REG_X86_OF), "Clears overflow flag");
        this->clearFlag_s(inst, architecture->getRegister(ID_REG_X86_PF), "Clears parity flag");
        this->clearFlag_s(inst, architecture->getRegister(ID_REG_X86_SF), "Clears sign flag");
        this->zf_s(inst, expr1, src1, true);
      ...
      void x86Semantics::pushal_s(triton::arch::Instruction& inst) {
        ...
        auto src1       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EAX));
        auto src2       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ECX));
        auto src3       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EDX));
        auto src4       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EBX));
        auto src5       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ESP));
        auto src6       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EBP));
        auto src7       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ESI));
        auto src8       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EDI));
      ...
      void x86Semantics::pushfd_s(triton::arch::Instruction& inst) {
        ...
        auto src1       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_CF));
        auto src2       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_PF));
        auto src3       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AF));
        auto src4       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ZF));
        auto src5       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_SF));
        auto src6       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_TF));
        auto src7       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_IF));
        auto src8       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_DF));
        auto src9       = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_OF));
      ...
      void x86Semantics::rcl_s(triton::arch::Instruction& inst) {
        auto& dst   = inst.operands[0];
        auto& src   = inst.operands[1];
        auto  srcCf = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_CF));
      ...
      void x86Semantics::rdtsc_s(triton::arch::Instruction& inst) {
        auto dst1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EDX));
        auto dst2 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EAX));
      ...
      void x86Semantics::sahf_s(triton::arch::Instruction& inst) {
        auto dst1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_SF));
        auto dst2 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_ZF));
        auto dst3 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AF));
        auto dst4 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_PF));
        auto dst5 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_CF));
        auto src  = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_AH));
      ...
      void x86Semantics::scasb_s(triton::arch::Instruction& inst) {
        auto& dst    = inst.operands[0];
        auto& src    = inst.operands[1];
        auto  index  = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_DI));
        auto  cx     = triton::arch::OperandWrapper(architecture->getParentRegister(ID_REG_X86_CX));
        auto  df     = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_DF));
      ...
      void x86Semantics::stc_s(triton::arch::Instruction& inst) {
        this->setFlag_s(inst, architecture->getRegister(ID_REG_X86_CF), "Sets carry flag");
        /* Upate the symbolic control flow */
        this->controlFlow_s(inst);
      }
      ...
      void x86Semantics::std_s(triton::arch::Instruction& inst) {
        this->setFlag_s(inst, architecture->getRegister(ID_REG_X86_DF), "Sets direction flag");
        /* Upate the symbolic control flow */
        this->controlFlow_s(inst);
      }
      ...
      void x86Semantics::sti_s(triton::arch::Instruction& inst) {
        this->setFlag_s(inst, architecture->getRegister(ID_REG_X86_IF), "Sets interrupt flag");
        /* Upate the symbolic control flow */
        this->controlFlow_s(inst);
      }
      ...
      void x86Semantics::stmxcsr_s(triton::arch::Instruction& inst) {
        auto& dst = inst.operands[0];
        auto  src = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_MXCSR));
      ...
      void x86Semantics::syscall_s(triton::arch::Instruction& inst) {
        auto dst1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RCX));
        auto dst2 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_R11));
        auto src1 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_RIP));
        auto src2 = triton::arch::OperandWrapper(architecture->getRegister(ID_REG_X86_EFLAGS));
