 ...
 */
    ...
    class API {
      ...
      public:
        ...
        std::set<triton::engines::taint::TagType*> getTags(const triton::arch::Register& reg);

        //! [**taint api**] - Retrieve the tags assigned to a MemoryAccess
        std::set<triton::engines::taint::TagType*> getTags(const triton::arch::MemoryAccess& mem);

        //! [**taint api**] - Retrieve the tags assigned to a memory address
        std::set<triton::engines::taint::TagType*> getTags(const triton::uint64 addr, const triton::uint32 size);
    };
