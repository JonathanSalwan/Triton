   ...
   */
    ...
    class Instruction {
      protected:
        ...
        std::set<std::pair<triton::arch::MemoryAccess, triton::engines::symbolic::SharedSymbolicExpression>> loadAccess;

        //! Implicit and explicit store access (write). This field is set at the semantics level.
        std::set<std::pair<triton::arch::MemoryAccess, triton::engines::symbolic::SharedSymbolicExpression>> storeAccess;

        //! Implicit and explicit register inputs (read). This field is set at the semantics level.
        std::set<std::pair<triton::arch::Register, triton::engines::symbolic::SharedSymbolicExpression>> readRegisters;

        //! Implicit and explicit register outputs (write). This field is set at the semantics level.
        std::set<std::pair<triton::arch::Register, triton::engines::symbolic::SharedSymbolicExpression>> writtenRegisters;

        //! Implicit and explicit immediate inputs (read). This field is set at the semantics level.
        std::set<std::pair<triton::arch::Immediate, triton::engines::symbolic::SharedSymbolicExpression>> readImmediates;
    ...
    private:
      ...
      public:
        ...
        TRITON_EXPORT std::set<std::pair<triton::arch::MemoryAccess, triton::engines::symbolic::SharedSymbolicExpression>>& getLoadAccess(void);

        //! Returns the list of all implicit and explicit store access
        TRITON_EXPORT std::set<std::pair<triton::arch::MemoryAccess, triton::engines::symbolic::SharedSymbolicExpression>>& getStoreAccess(void);

        //! Returns the list of all implicit and explicit register (flags includes) inputs (read)
        TRITON_EXPORT std::set<std::pair<triton::arch::Register, triton::engines::symbolic::SharedSymbolicExpression>>& getReadRegisters(void);

        //! Returns the list of all implicit and explicit register (flags includes) outputs (write)
        TRITON_EXPORT std::set<std::pair<triton::arch::Register, triton::engines::symbolic::SharedSymbolicExpression>>& getWrittenRegisters(void);

        //! Returns the list of all implicit and explicit immediate inputs (read)
        TRITON_EXPORT std::set<std::pair<triton::arch::Immediate, triton::engines::symbolic::SharedSymbolicExpression>>& getReadImmediates(void);
        ...
        //! Sets a load access.
        TRITON_EXPORT const triton::engines::symbolic::SharedSymbolicExpression& setLoadAccess(const triton::arch::MemoryAccess& mem, const triton::engines::symbolic::SharedSymbolicExpression& expr);

        //! Removes a load access.
        TRITON_EXPORT void removeLoadAccess(const triton::arch::MemoryAccess& mem);

        //! Sets a store access.
        TRITON_EXPORT const triton::engines::symbolic::SharedSymbolicExpression& setStoreAccess(const triton::arch::MemoryAccess& mem, const triton::engines::symbolic::SharedSymbolicExpression& expr);

        //! Removes a store access.
        TRITON_EXPORT void removeStoreAccess(const triton::arch::MemoryAccess& mem);

        //! Sets a read register.
        TRITON_EXPORT const triton::engines::symbolic::SharedSymbolicExpression& setReadRegister(const triton::arch::Register& reg, const triton::engines::symbolic::SharedSymbolicExpression& expr);

        //! Removes a read register.
        TRITON_EXPORT void removeReadRegister(const triton::arch::Register& reg);

        //! Sets a written register.
        TRITON_EXPORT const triton::engines::symbolic::SharedSymbolicExpression& setWrittenRegister(const triton::arch::Register& reg, const triton::engines::symbolic::SharedSymbolicExpression& expr);

        //! Removes a written register.
        TRITON_EXPORT void removeWrittenRegister(const triton::arch::Register& reg);

        //! Sets a read immediate.
        TRITON_EXPORT const triton::engines::symbolic::SharedSymbolicExpression& setReadImmediate(const triton::arch::Immediate& imm, const triton::engines::symbolic::SharedSymbolicExpression& expr);
