  ...
  namespace ast {
    namespace representations {
      ...
      std::ostream& AstSmtRepresentation::print(std::ostream& stream, triton::ast::DeclareNode* node) {
        ...
        stream << "(declare-fun " << var->getName();
        stream << "_" << var->getSize();
        auto comment = var->getAlnumComment();
        if (!comment.empty())
          stream << "_" << comment;
        stream << " () (_ BitVec " << var->getSize() << "))";
        return stream;
      }
      ...
      std::ostream& AstSmtRepresentation::print(std::ostream& stream, triton::ast::VariableNode* node) {
        ...
        stream << "_" << node->getVar()->getSize();
        auto comment = node->getVar()->getAlnumComment();
        if (!comment.empty())
          stream << "_" << comment;
        return stream;
      }
