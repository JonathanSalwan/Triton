...
namespace triton {
  namespace ast {
    static uint32_t depth_ = 0;
    static uint32_t step_ = 0;
    static bool logging = Z3_open_log("/tmp/z3.log");
    class recursionTracker {
      public:
        recursionTracker() {
          step_++;
          depth_++;
          printf("[Debug] Step %d: Reaching recursion depth %d\n", step_, depth_);
        }
        ~recursionTracker() {
          depth_--;
          printf("[Debug] Reducing recursion depth to %d\n", depth_);
        }
    };
