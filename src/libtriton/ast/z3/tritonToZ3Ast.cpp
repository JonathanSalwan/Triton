...
#include <tritonast/tritonToZ3Ast.hpp>
  ...
  //      std::cout << p.first << "/" << p.second << "/" << p.first->getChildren().size() << "\n";
    ...
    z3::expr TritonToZ3Ast::do_convert(triton::ast::AbstractNode* node, std::stack<z3::expr> & childs) {
      switch (node->getKind()) {
        ...
        case BVROL_NODE: {
          uint32_t op1 = pop_back(childs).get_numeral_uint();
        ...
        case BV_NODE: {
          ...
          uint32_t bvsize = static_cast<uint32_t>(this->getUintValue(size));
        ...
        case CONCAT_NODE: {
          ...
          for (uint32_t idx = 1; idx < children.size(); idx++) {
        ...
        case EXTRACT_NODE: {
          ...
          z3::expr value    = pop_back(childs);
          uint32_t hv = static_cast<uint32_t>(this->getUintValue(high));
          uint32_t lv = static_cast<uint32_t>(this->getUintValue(low));
...
//          this->symbols[symbol] = node->getChildren()[1].get();
//
//          return this->convert(node->getChildren()[2].get());
//        }
        ...
        case REFERENCE_NODE:
           // TODO Save computed reference
          return this->convert(reinterpret_cast<triton::ast::ReferenceNode*>(node)->getAst().get());

//        case STRING_NODE: {
//                            // TODO Handle symboles
...
//        }
        ...
        case SX_NODE: {
          ...
          z3::expr value      = pop_back(childs);
          uint32_t extv = static_cast<uint32_t>(this->getUintValue(ext));
