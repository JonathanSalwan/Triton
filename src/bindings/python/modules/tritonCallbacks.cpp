...
namespace PyTritonOptions {
  ...
  std::set<reg_size>  startAnalysisFromAddr;
  std::set<reg_size>  startAnalysisFromOffset;
  std::set<reg_size>  stopAnalysisFromAddr;
  std::set<reg_size>  stopAnalysisFromOffset;
  ...
  std::map<reg_size, std::list<reg_size>> taintRegFromAddr;   // <addr, [reg1, reg2]>
  std::map<reg_size, std::list<reg_size>> untaintRegFromAddr; // <addr, [reg1, reg2]>
  std::map<reg_size, std::list<reg_size>> taintMemFromAddr;   // <addr, [mem1, mem2]>
  std::map<reg_size, std::list<reg_size>> untaintMemFromAddr; // <addr, [mem1, mem2]>
};
...
static PyObject *Triton_addCallback(PyObject *self, PyObject *args) {
  ...
  if (PyLong_AsLongLong(flag) == CB_BEFORE)
    PyTritonOptions::callbackBefore = function;

  else if ((PyLong_AsLongLong(flag) == CB_BEFORE_SYMPROC))
    PyTritonOptions::callbackBeforeIRProc = function;

  else if ((PyLong_AsLongLong(flag) == CB_AFTER))
    PyTritonOptions::callbackAfter = function;

  else if ((PyLong_AsLongLong(flag) == CB_FINI))
    PyTritonOptions::callbackFini = function;

  else if ((PyLong_AsLongLong(flag) == CB_SIGNALS))
    PyTritonOptions::callbackSignals = function;

  else if ((PyLong_AsLongLong(flag) == CB_SYSCALL_ENTRY))
    PyTritonOptions::callbackSyscallEntry = function;

  else if ((PyLong_AsLongLong(flag) == CB_SYSCALL_EXIT))
    PyTritonOptions::callbackSyscallExit = function;

  else if (PyLong_AsLongLong(flag) == CB_IMAGE_LOAD)
    PyTritonOptions::callbackImageLoad = function;

  else if ((PyLong_AsLongLong(flag) == CB_ROUTINE_ENTRY)){
  ...
  else if ((PyLong_AsLongLong(flag) == CB_ROUTINE_EXIT)){
    if (routine == nullptr || !PyString_Check(routine))
      return PyErr_Format(PyExc_TypeError, "addCallback(): expected a routine name (string) as third argument");
...
static PyObject *Triton_getRegName(PyObject *self, PyObject *reg) {
  ...
  return Py_BuildValue("s", PINConverter::getRegisterName(PyLong_AsLongLong(reg)).c_str());
}
...
static PyObject *Triton_getFlagValue(PyObject *self, PyObject *flagId) {
  ...
  flag = createTmpFlag(PyLong_AsLongLong(flagId));
...
static PyObject *Triton_getSyscallNumber(PyObject *self, PyObject *std) {
  ...
  return PyLong_FromLongLong(syscallNumber);
}
...
static PyObject *Triton_setMemValue(PyObject *self, PyObject *args) {
  ...
  ad = PyLong_AsLongLong(addr);
  ws = PyLong_AsLongLong(writeSize);
...
static PyObject *Triton_setRegValue(PyObject *self, PyObject *args) {
  ...
  if (isSSERegId(tr))
    ...
    ap.setRegisterValue(ro, boost::numeric_cast<reg_size>(va));
...
static PyObject *Triton_startAnalysisFromAddr(PyObject *self, PyObject *addr) {
  ...
  PyTritonOptions::startAnalysisFromAddr.insert(PyLong_AsLongLong(addr));
  Py_INCREF(Py_None);
  return Py_None;
}
...
static PyObject *Triton_startAnalysisFromOffset(PyObject *self, PyObject *offset) {
  ...
  PyTritonOptions::startAnalysisFromOffset.insert(PyLong_AsLongLong(offset));
  Py_INCREF(Py_None);
  return Py_None;
}
...
static PyObject *Triton_stopAnalysisFromOffset(PyObject *self, PyObject *offset) {
  ...
  PyTritonOptions::stopAnalysisFromOffset.insert(PyLong_AsLongLong(offset));
  Py_INCREF(Py_None);
  return Py_None;
}
...
static PyObject *Triton_concretizeMem(PyObject *self, PyObject *addr) {
  ...
  MemoryOperand mem(PyLong_AsLongLong(addr), 1);
...
static PyObject *Triton_concretizeReg(PyObject *self, PyObject *regId) {
  ...
  RegisterOperand reg = createTmpReg(PyLong_AsLongLong(regId));
...
static PyObject *Triton_convertExprToSymVar(PyObject *self, PyObject *args) {
  ...
  ei = PyLong_AsLongLong(exprId);
  vs = PyLong_AsLongLong(symVarSize);
...
static PyObject *Triton_convertMemToSymVar(PyObject *self, PyObject *args) {
  ...
  vs = PyLong_AsLongLong(symVarSize);
  vc = PyString_AsString(varComment);
  MemoryOperand mo(PyLong_AsLongLong(memAddr), vs);
...
static PyObject *Triton_convertRegToSymVar(PyObject *self, PyObject *args) {
  ...
  vs = PyLong_AsLongLong(symVarSize);
  vc = PyString_AsString(varComment);
  RegisterOperand ro = createTmpReg(PyLong_AsLongLong(regId));
...
static PyObject *Triton_getModels(PyObject *self, PyObject *args) {
  ...
  limit_c = PyLong_AsLongLong(limit);
  if (limit_c == 0)
  ...
  for (reg_size index = 0; index < modelsSize; index++){
...
static PyObject *Triton_getPathConstraints(PyObject *self, PyObject *noargs) {
  PyObject                        *ppc;
  std::list<reg_size>               pc;
  std::list<reg_size>::iterator     it;
  Py_ssize_t                      size = 0;
...
static PyObject *Triton_getRegs(PyObject *self, PyObject *noargs) {
  ...
  for (reg_size regId = ID_RAX; regId < ID_RFLAGS; regId++){
  #endif

  #if defined(__i386) || defined(_M_IX86)
  for (reg_size regId = ID_EAX; regId < ID_EFLAGS; regId++){ 
  #endif
  ...
  for (reg_size flagId = ID_AF; flagId <= ID_ZF; flagId++){
...
static PyObject *Triton_getStats(PyObject *self, PyObject *noargs) {
  ...
  PyDict_SetItemString(stats, "branches",     PyLong_FromLongLong(ap.getNumberOfBranchesTaken()));
  PyDict_SetItemString(stats, "expressions",  PyLong_FromLongLong(ap.getNumberOfExpressions()));
  PyDict_SetItemString(stats, "time",         PyLong_FromLongLong(ap.getTimeOfExecution()));
  PyDict_SetItemString(stats, "unknownExpr",  PyLong_FromLongLong(ap.getNumberOfUnknownInstruction()));
  return stats;
}
...
static PyObject *Triton_getSymVar(PyObject *self, PyObject *symVarPy) {
  ...
  if (PyLong_Check(symVarPy) || PyInt_Check(symVarPy))
    symVar = ap.getSymVar(PyLong_AsLongLong(symVarPy));
  else
...
static PyObject *Triton_taintMemFromAddr(PyObject *self, PyObject *args) {
  ...
  for (Py_ssize_t i = 0; i < PyList_Size(mems); i++){
    ...
    memsList.push_back(PyLong_AsLongLong(item));
  }

  /* Update taint configuration */
  PyTritonOptions::taintMemFromAddr.insert(std::pair<reg_size, std::list<reg_size>>(PyLong_AsLongLong(addr), memsList));
  Py_INCREF(Py_None);
  return Py_None;
}
...
static PyObject *Triton_taintRegFromAddr(PyObject *self, PyObject *args) {
  ...
  for (Py_ssize_t i = 0; i < PyList_Size(regs); i++){
    ...
    regsList.push_back(PyLong_AsLongLong(item));
  }

  /* Update taint configuration */
  PyTritonOptions::taintRegFromAddr.insert(std::pair<reg_size, std::list<reg_size>>(PyLong_AsLongLong(addr), regsList));
  Py_INCREF(Py_None);
  return Py_None;
}
...
static PyObject *Triton_untaintMemFromAddr(PyObject *self, PyObject *args) {
  ...
  /* Update taint configuration */
  PyTritonOptions::untaintMemFromAddr.insert(std::pair<reg_size, std::list<reg_size>>(PyLong_AsLongLong(addr), memsList));
...
static PyObject *Triton_untaintRegFromAddr(PyObject *self, PyObject *args) {
  ...
  /* Update taint configuration */
  PyTritonOptions::untaintRegFromAddr.insert(std::pair<reg_size, std::list<reg_size>>(PyLong_AsLongLong(addr), regsList));
