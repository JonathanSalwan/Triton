...
/**
 *  We decided to use our own types as we use bigint (uint512 for example)
 *  while this is not standard. So we have to define this new type from boost
 *  but if we call it triton::uint512, it may conflict with types from others libraries.
 *  To keep the same syntax for every type, we use this triton namespace.
 */
 ...
 */
    ...
    //! Max number of bits supported
    constexpr size_t max_bits_supported = 512;
