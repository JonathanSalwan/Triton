...
DEBUG  = False
# DEBUG  = True
TARGET = os.path.join(os.path.dirname(__file__), './bin/crypto_test-thumb-Oz.bin')
STOP_ADDR = None
MAX_INSTRS = 100000
...
# Simulate the aeabi_memclr8() function
def aeabi_memclr8Handler(ctx):
    print('[+] __aeabi_memclr8 hooked (calling __aeabi_memclr)')

    aeabi_memclrHandler(ctx)
...
customRelocation = [
    ...
    ('__aeabi_memcpy',  aeabi_memcpyHandler,  BASE_PLT + 2 * 8),
    ('__aeabi_memclr8', aeabi_memclr8Handler, BASE_PLT + 3 * 8),
    ('__aeabi_memclr',  aeabi_memclrHandler,  BASE_PLT + 4 * 8),
]
