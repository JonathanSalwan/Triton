...
DEBUG  = False
# DEBUG  = True
TARGET = os.path.join(os.path.dirname(__file__), './bin/crypto_test-thumb-O3.bin')
STOP_ADDR = None
MAX_INSTRS = 100000
...
customRelocation = [
    ...
    ('__aeabi_memclr8', aeabi_memclr8Handler, BASE_PLT + 2 * 8),
    ...
    while pc and count < MAX_INSTRS:
        ...
        if pc == 0x820:
            ...
            if zf == 1:
                ...
                pc += 2 + 2 + 2 + 2 # skip next 3 instructions
            ctx.setConcreteRegisterValue(ctx.registers.pc, pc)
            continue
        ...
        if pc == 0xf80:
            ...
            if zf == 1:
                ...
                pc += 2 + 4 + 2 + 4 # skip next 3 instructions
            ctx.setConcreteRegisterValue(ctx.registers.pc, pc)
            continue
