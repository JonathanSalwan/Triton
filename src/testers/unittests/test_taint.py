...
class TestTaint(unittest.TestCase):
    ...
    def test_known_issues(self):
        ...
        Triton.taintRegister(Triton.getRegister(REG.X86_64.EAX))
        inst = Instruction()
        # lea eax,[esi+eax*1]
        inst.setOpcode("\x8D\x04\x06")
        Triton.processing(inst)

        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.EAX)))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.EBX)))
    ...
    def test_taint_register(self):
        ...
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        Triton.taintRegister(Triton.getRegister(REG.X86_64.RAX))
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        Triton.untaintRegister(Triton.getRegister(REG.X86_64.RAX))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))

        Triton.taintRegister(Triton.getRegister(REG.X86_64.AH))
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.EAX)))
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.AX)))

        Triton.untaintRegister(Triton.getRegister(REG.X86_64.AH))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.EAX)))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.AX)))
    ...
    def test_taint_assignement_memory_register(self):
        ...
        Triton.taintAssignmentMemoryRegister(MemoryAccess(0x2002, 2), Triton.getRegister(REG.X86_64.AX))
        self.assertTrue(Triton.isMemoryTainted(MemoryAccess(0x2000, 1)))
        self.assertTrue(Triton.isMemoryTainted(MemoryAccess(0x2001, 1)))
        self.assertFalse(Triton.isMemoryTainted(MemoryAccess(0x2002, 1)))
        self.assertFalse(Triton.isMemoryTainted(MemoryAccess(0x2003, 1)))
        ...
        Triton.taintAssignmentMemoryRegister(MemoryAccess(0x1fff, 8), Triton.getRegister(REG.X86_64.RAX))
        self.assertFalse(Triton.isMemoryTainted(MemoryAccess(0x1fff, 1)))
        self.assertFalse(Triton.isMemoryTainted(MemoryAccess(0x2000, 1)))
        self.assertFalse(Triton.isMemoryTainted(MemoryAccess(0x2001, 1)))
        self.assertFalse(Triton.isMemoryTainted(MemoryAccess(0x2002, 1)))
    ...
    def test_taint_assignement_register_immediate(self):
        ...
        Triton.taintAssignmentRegisterImmediate(Triton.getRegister(REG.X86_64.RAX))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
    ...
    def test_taint_assignement_register_memory(self):
        ...
        Triton.taintAssignmentRegisterMemory(Triton.getRegister(REG.X86_64.RAX), MemoryAccess(0x2000, 8))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        ...
        Triton.taintAssignmentRegisterMemory(Triton.getRegister(REG.X86_64.RAX), MemoryAccess(0x3000, 8))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
    ...
    def test_taint_assignement_register_register(self):
        ...
        Triton.taintAssignmentRegisterRegister(Triton.getRegister(REG.X86_64.RAX), Triton.getRegister(REG.X86_64.RAX))
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        ...
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RBX)))
        Triton.taintRegister(Triton.getRegister(REG.X86_64.RBX))
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RBX)))

        Triton.taintAssignmentRegisterRegister(Triton.getRegister(REG.X86_64.RAX), Triton.getRegister(REG.X86_64.RBX))
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
    ...
    def test_taint_union_memory_register(self):
        ...
        Triton.taintUnionMemoryRegister(MemoryAccess(0x2000, 4), Triton.getRegister(REG.X86_64.RAX))
        self.assertTrue(Triton.isMemoryTainted(MemoryAccess(0x2000, 4)))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
    ...
    def test_taint_union_register_immediate(self):
        ...
        Triton.taintUnionRegisterImmediate(Triton.getRegister(REG.X86_64.RAX))
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
    ...
    def test_taint_union_register_memory(self):
        ...
        Triton.taintUnionRegisterMemory(Triton.getRegister(REG.X86_64.RAX), MemoryAccess(0x2000, 4))
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        self.assertFalse(Triton.isMemoryTainted(MemoryAccess(0x2000, 4)))
    ...
    def test_taint_union_register_register(self):
        ...
        Triton.taintUnionRegisterRegister(Triton.getRegister(REG.X86_64.RAX), Triton.getRegister(REG.X86_64.RBX))
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RBX)))
        ...
        Triton.untaintRegister(Triton.getRegister(REG.X86_64.RAX))
        Triton.untaintRegister(Triton.getRegister(REG.X86_64.RBX))
        Triton.taintUnionRegisterRegister(Triton.getRegister(REG.X86_64.RAX), Triton.getRegister(REG.X86_64.RBX))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RBX)))
    ...
    def test_taint_get_tainted_registers(self):
        ...
        Triton.taintRegister(Triton.getRegister(REG.X86_64.EAX))
        Triton.taintRegister(Triton.getRegister(REG.X86_64.AX))
        Triton.taintRegister(Triton.getRegister(REG.X86_64.RBX))
        Triton.taintRegister(Triton.getRegister(REG.X86_64.CL))
        Triton.taintRegister(Triton.getRegister(REG.X86_64.DI))

        r = Triton.getTaintedRegisters()
        self.assertTrue(Triton.getRegister(REG.X86_64.RAX) in r)
        self.assertTrue(Triton.getRegister(REG.X86_64.RBX) in r)
        self.assertTrue(Triton.getRegister(REG.X86_64.RCX) in r)
        self.assertTrue(Triton.getRegister(REG.X86_64.RDI) in r)
    ...
    def test_taint_set_register(self):
        ...
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        Triton.setTaintRegister(Triton.getRegister(REG.X86_64.RAX), True)
        self.assertTrue(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
        Triton.setTaintRegister(Triton.getRegister(REG.X86_64.RAX), False)
        self.assertFalse(Triton.isRegisterTainted(Triton.getRegister(REG.X86_64.RAX)))
