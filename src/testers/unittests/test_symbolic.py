...
class TestSymbolic(unittest.TestCase):
    ...
    def test_backup(self):
        ...
        * Also check reseting a disable symbolic engines doesn't crash.
        ...
        self.assertEqual(self.Triton.getSymbolicRegisterValue(self.Triton.Register(REG.RAX)), 1)
        ...
        self.assertEqual(self.Triton.getConcreteRegisterValue(self.Triton.Register(REG.RAX)), 2, "concrete value is updated")
        self.assertEqual(self.Triton.getSymbolicRegisterValue(self.Triton.Register(REG.RAX)), 1)
        self.assertEqual(self.Triton.getSymbolicRegisterValue(self.Triton.Register(REG.RAX)), 1, "Symbolic value is not update")
    ...
    def test_bind_expr_to_register(self):
        ...
        self.Triton.assignSymbolicExpressionToRegister(expr1, self.Triton.Register(REG.RAX))

        self.assertEqual(self.Triton.getSymbolicRegisterValue(self.Triton.Register(REG.RAX)), 0x11223344)
...
class TestSymbolicBuilding(unittest.TestCase):
    ...
    def test_build_register(self):
        ...
        node = self.Triton.buildSymbolicRegister(self.Triton.Register(REG.RAX))
        self.assertEqual(node.evaluate(), 0x1122334455667788)
        self.assertEqual(node.getBitvectorSize(), CPUSIZE.QWORD_BIT)

        node = self.Triton.buildSymbolicRegister(self.Triton.Register(REG.EAX))
        self.assertEqual(node.evaluate(), 0x55667788)
        self.assertEqual(node.getBitvectorSize(), CPUSIZE.DWORD_BIT)

        node = self.Triton.buildSymbolicRegister(self.Triton.Register(REG.AX))
        self.assertEqual(node.evaluate(), 0x7788)
        self.assertEqual(node.getBitvectorSize(), CPUSIZE.WORD_BIT)

        node = self.Triton.buildSymbolicRegister(self.Triton.Register(REG.AH))
        self.assertEqual(node.evaluate(), 0x77)
        self.assertEqual(node.getBitvectorSize(), CPUSIZE.BYTE_BIT)

        node = self.Triton.buildSymbolicRegister(self.Triton.Register(REG.AL))
