...
class TestX86Arch(unittest.TestCase):
    ...
    def test_registers(self):
        ...
        with self.assertRaises(Exception):
            self.Triton.getRegister(REG.X86_64.RAX).getName()

        with self.assertRaises(Exception):
            self.Triton.getRegister(REG.X86_64.ZMM1).getName()

        with self.assertRaises(Exception):
            self.Triton.getRegister(REG.X86_64.XMM8).getName()

        with self.assertRaises(Exception):
            self.Triton.getRegister(REG.X86_64.XMM15).getName()

        self.assertEqual(self.Triton.getRegister(REG.X86_64.XMM7).getName(), "xmm7")
...
class TestX8664Arch(unittest.TestCase):
    ...
    def test_registers(self):
        ...
        self.assertEqual(self.Triton.getRegister(REG.X86_64.RAX).getName(), "rax")
        self.assertEqual(self.Triton.getRegister(REG.X86_64.ZMM1).getName(), "zmm1")
        self.assertEqual(self.Triton.getRegister(REG.X86_64.XMM15).getName(), "xmm15")
