...
AdcIRBuilder::AdcIRBuilder(__uint address, const std::string &disassembly):
  BaseIRBuilder(address, disassembly) {
}


void AdcIRBuilder::regImm(Inst &inst) const {
  ...
  EflagsBuilder::cfAdd(inst, se, reg, op1);
...
void AdcIRBuilder::regReg(Inst &inst) const {
  SymbolicExpression *se;
  smt2lib::smtAstAbstractNode *expr, *op1, *op2, *op3;
  ...
  EflagsBuilder::cfAdd(inst, se, reg1, op1);
  EflagsBuilder::ofAdd(inst, se, reg1, op1, op2);
...
void AdcIRBuilder::regMem(Inst &inst) const {
  SymbolicExpression *se;
  smt2lib::smtAstAbstractNode *expr, *op1, *op2, *op3;
...
void AdcIRBuilder::memImm(Inst &inst) const {
  SymbolicExpression *se;
  smt2lib::smtAstAbstractNode *expr, *op1, *op2, *op3;
  ...
  EflagsBuilder::cfAdd(inst, se, mem, op1);
...
void AdcIRBuilder::memReg(Inst &inst) const {
  SymbolicExpression *se;
  smt2lib::smtAstAbstractNode *expr, *op1, *op2, *op3;
  auto memSize = this->operands[0].getMem().getSize();
  auto mem = this->operands[0].getMem();
...
Inst *AdcIRBuilder::process(void) const {
  this->checkSetup();

  Inst *inst = new Inst(ap.getThreadID(), this->address, this->disas);

  try {
    this->templateMethod(*inst, this->operands, "ADC");
