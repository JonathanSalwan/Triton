...
def printfAnalysis(threadId):
    ...
    index = 0
    while getMemValue(arg + index, IDREF.CPUSIZE.BYTE_BIT) != 0x00:
...
def mainAnalysis(threadId):
    ...
    while rdi != 0:
        argv = getMemValue(rsi + ((rdi-1) * IDREF.CPUSIZE.REG), IDREF.CPUSIZE.REG_BIT)
        offset = 0
        while getMemValue(argv + offset, IDREF.CPUSIZE.BYTE_BIT) != 0x00:
